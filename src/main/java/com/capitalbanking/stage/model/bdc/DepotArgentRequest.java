...
public class DepotArgentRequest {
    ...
    @Schema(description = "Opération demandée. Valeur fixe: direct_cash_in",
            example = "direct_cash_in", requiredMode = Schema.RequiredMode.REQUIRED)
    private String intent;

    @NotBlank(message = "amount is required")
    @Pattern(regexp = "^\\d+(\\.\\d+)?$", message = "amount must be a valid positive number")
    @Schema(description = "Montant de la transaction.",
            example = "3500.0", requiredMode = Schema.RequiredMode.REQUIRED)
    ...
    @Schema(description = "Horodatage de création de la transaction (epoch ms).",
            example = "1746416166000", requiredMode = Schema.RequiredMode.REQUIRED)
    private String createtime;

    @NotBlank(message = "issuertrxref is required")
    @Schema(description = "Référence unique de transaction émise par le Switch.",
            example = "948488604871", requiredMode = Schema.RequiredMode.REQUIRED)
